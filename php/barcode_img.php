...
if ( !isset( $_GET [ 'number' ] ) ) {
  ...
  //$font     = 'barcode/sample php/font/NOTTBL__.TTF';
  // - -
  ...
  $y        = 35;  // barcode center
  $height   = 70;   // barcode height in 1D ; module size in 2D
  $width    = 2;    // barcode height in 1D ; not use in 2D
  $angle    = 0;   // rotation in degrees : nb : non horizontable barcode might not be usable because of pixelisation
  
  $code     = $_GET [ 'number' ]; // barcode, of course ;)
  $type     = 'code128';
  ...
  // -------------------------------------------------- //
  $im     = imagecreatetruecolor(350, 70);
  $black  = ImageColorAllocate($im,0x00,0x00,0x00);
  $white  = ImageColorAllocate($im,0xff,0xff,0xff);
  $red    = ImageColorAllocate($im,0xff,0x00,0x00);
  $blue   = ImageColorAllocate($im,0x00,0x00,0xff);
  imagefilledrectangle($im, 0, 0, 350, 70, $white);
  ...
  //imagettftext($im, $fontSize, $angle, $x + 300 + 10, $y + 10, $blue, $font, $data['hri']);
  //imagettftext($im, 30, 0, 60, 100, $black, $font, 'BARCODE ROTATION : 90�');
  // -------------------------------------------------- //
  //                     ROTATE
  // -------------------------------------------------- //
